...
- **认证演进备注（2026-08-29）**：当前锁定的 `dsh@0.1.1-rc.2` 无内置认证（README 安全警示基于此）；上游 0.1.2-alpha.1 起提供 launch token + 签名 Cookie 浏览器认证链（实测该版本仅有 GitHub tag、未上 npm，npm latest 仍为 0.1.1-rc.2）。升级锁定版本时，`/api/mini/*` 需一并纳入同一信任栅栏（Cookie 校验或经栅栏内注册），`client.js` 的 ws 连接亦需携带会话凭据。
