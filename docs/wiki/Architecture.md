...
│                focus 惰性转发 dsh-plugins-norm（缺席回 503）   │
│                （client 脸、/api/mini/ws 与 9 包 inject 已移除）│
├─ dsh-plugins-norm ≥1.0.1（家族漂移屏蔽层，独立仓库）──────────┤
│ 稳定面  GET /api/dsh-plugins-norm/caps（部署体检锚点）         │
│         POST /api/dsh-plugins-norm/focus（跳转广播+审计）      │
│         /api/dsh-plugins-norm/ws 被动事件通道                  │
│ client 侧 window.__dshPluginsNorm 微内核（on/caps/focusSession，│
...
→ 收起胶囊 → NSWorkspace 打开主应用
   主应用 WebView 就绪后 window.__dshPluginsNorm 接收跳转广播
   → 会话跳转三级降级链 → ctx.sessions.open(sessionId)（官方侧边栏点击同款调用：选中 + 展示全链路）
```

## 4. 关键设计决策

- **会话跳转执行移交 dsh-plugins-norm（mini-dialog 0.2.0 起）**：官方 client 侧接口只允许一个插件触碰——家族漂移屏蔽层 norm（仓库 [iiiiiei/dsh-plugin-norm](https://github.com/iiiiiei/dsh-plugin-norm)，包名/服务名/路由前缀为 `dsh-plugins-norm`）持有浏览器端执行面：`window.__dshPluginsNorm` 微内核（on / caps / focusSession，会话跳转三级降级链）。mini-dialog 因此退役 client 脸（含 9 包 inject 声明）与自有 `/api/mini/ws` 通道，`/api/mini/focus` 改经 norm 稳定面广播（`POST /api/dsh-plugins-norm/focus`，含 lastFocusResult 审计）；norm 缺席时惰性解析落空、回 503 部署指引，不把插件拖进 fail-fast。norm 另提供 `GET /api/dsh-plugins-norm/caps`（宿主形状探测 + 客户端上报合并 + 降级清单，免认证）兼作部署体检锚点与 `/api/dsh-plugins-norm/ws` 被动事件通道；全程零轮询零缓存。
...
mini-dialog 是**套件级组件**：它的宿主服务跑在 DSH 后端（v0.2.0 起宿主单脸，浏览器端执行面在 dsh-plugins-norm 的 client 微内核里），但它的唯一调用方是 Launcher 的迷你框。归属与分发据此设计：
...
- **前置依赖（v0.2.0 起）**：会话跳转通道前置 **dsh-plugins-norm ≥ 1.0.1**（独立仓库，见第 4 节，不随 Launcher Release 分发）；装卸必须与 cordis.patch.yml 条目同步——patch 引用了 node_modules 里缺失的包时，整个 profile 会拒绝启动
