...
- **一键体检**：终端风只读体检窗，十一项检测逐行刷出，按结果给出条件动作按钮；报告头新增「版本方框」——前端（壳）/后端 dsh/已装家族插件（theme-sdk、plugin-norm、mini-dialog、l10n-zh、theme-grok）版本一目了然，未安装的插件自动隐去
- **状态钮零无谓重绘**：状态与提示未变化时不重写菜单栏图标（AppKit 无同值比较），自适应轮询稳态 30s、变化后 60s 内加密 5s
